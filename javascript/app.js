...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Логика для смены картинок в галерее конкурса при наведении ---

    // Находим все элементы галереи, которые должны менять картинки.
    const cycleImageContainers = document.querySelectorAll('.image-cycle-on-hover');

    // Проходимся по каждому такому элементу.
    cycleImageContainers.forEach(container => {
        // Находим картинку внутри.
        const image = container.querySelector('img');
        // Если картинки нет, ничего не делаем.
        if (!image) return;

        // Запоминаем оригинальный путь к картинке, чтобы вернуть ее потом.
        const originalSrc = image.src;
        const cycleImagesAttr = image.dataset.cycleImages;

        // Если атрибута с картинками нет, выходим.
        if (!cycleImagesAttr) return;

        // Превращаем строку "путь1,путь2,путь3" в массив ["путь1", "путь2", "путь3"].
        const imagesToCycle = cycleImagesAttr.split(',');

        let currentIndex = 0;
        let intervalId = null; // Переменная для хранения ID таймера, чтобы его можно было остановить.

        // Когда мышка заходит на элемент...
        container.addEventListener('mouseenter', () => {
            intervalId = setInterval(() => {
                // Меняем картинку на следующую из нашего массива.
                image.src = imagesToCycle[currentIndex];
                // Увеличиваем счетчик. Если он дошел до конца массива, он вернется на 0.
                currentIndex = (currentIndex + 1) % imagesToCycle.length;
            }, 500);
        });

        // Когда мышка уходит с элемента...
        container.addEventListener('mouseleave', () => {
            // ...останавливаем таймер.
            clearInterval(intervalId);
            // Возвращаем самую первую, оригинальную картинку.
            image.src = originalSrc;
            // Сбрасываем счетчик на 0 для следующего наведения.
            currentIndex = 0;
        });
    });

    // --- Логика для анимированной галереи ---

    // Находим наш специальный анимированный блок.
    const animatedItem = document.querySelector('.animated-gallery-item');

    // Если такой блок на странице есть...
    if (animatedItem) {

        // --- 1. Логика для смены фона ---

        const backgrounds = [
            'url(../images/grma1.webp)',
            'url(../images/grma2.webp)'
        ];
        let currentBgIndex = 0;

        // Запускаем таймер, который будет менять фон каждые 2 секунды.
        setInterval(() => {
            // Переключаем индекс фона (если был 0, станет 1; если был 1, станет 0).
            currentBgIndex = 1 - currentBgIndex;
            // Применяем новый фон к нашему блоку.
            animatedItem.style.backgroundImage = backgrounds[currentBgIndex];
        }, 2000);


        // --- 2. Логика для прыгающего элемента ---

        // Находим сам прыгающий элемент.
        const bouncingEl = animatedItem.querySelector('.bouncing-element');

        let x = 0; // Текущая позиция по горизонтали.
        let y = 0; // Текущая позиция по вертикали.

        let dx = 1.5; // Скорость и направление по горизонтали.
        let dy = 1.5; // Скорость и направление по вертикали.

        // Создаем функцию, которая будет выполняться на каждом кадре анимации.
        function animateBounce() {
            // Получаем размеры контейнера и самого элемента.
            const containerWidth = animatedItem.clientWidth;
            const containerHeight = animatedItem.clientHeight;
            const elWidth = bouncingEl.clientWidth;
            const elHeight = bouncingEl.clientHeight;
            ...
            // Если ударились о левую или правую стенку...
            if (x <= 0 || x + elWidth >= containerWidth) {
                // ...меняем направление движения по горизонтали на противоположное.
                dx = -dx;
            }
            // Если ударились о верхнюю или нижнюю стенку...
            if (y <= 0 || y + elHeight >= containerHeight) {
                // ...меняем направление движения по вертикали на противоположное.
                dy = -dy;
            }

            // Применяем новые координаты к стилям элемента, заставляя его двигаться.
            bouncingEl.style.left = x + 'px';
            bouncingEl.style.top = y + 'px';

            // Просим браузер вызвать эту же функцию на следующем кадре.
            requestAnimationFrame(animateBounce);
        }

        // Запускаем нашу анимацию в первый раз.
        requestAnimationFrame(animateBounce);
    }
});